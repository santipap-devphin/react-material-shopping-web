...
const TagContent = () => {
    ...
    useEffect(() => {
        ...
        const callData = async() => {
            ...
            try {
                const response  = await Promise.all([endpoint.get(`/blogs/taggroup/${slug}`) , endpoint.get("/getcategory") , endpoint.get("/tags")])
     ...
     // ทำการ ส่ง param tag ไปที่ฝั่ง server แล้ว ให้เอา param ที่ส่งไป เช็คว่า Tagid อะไร จากนั้นให้เอาไป เช็คใน blog ว่า ข้อความไหนมีแท๊กนี้บ้าง


  return (<><Stack sx={{mb:1}}>
                <Typography variant='h6'>Tags : {slug}</Typography>
            </Stack>
